function output = AutoCalciumvsContraction_RepeatedStimulus_V180802(framesel,ref,SelectRef);
...
[image folder3] = uigetfile('*.*');
 quietG_push = multitiff2M([folder3 image],1);
    ...
    quietG_push = squeeze(std(Go,0,3));
...
Answer=input('Est ce ok?''1 si oui, 0 si non\n');
end
...
figure();
[Max,PosMax]=max(diff(TotalNoise));
plot(diff(TotalNoise))
title('Select highest peak and press enter')
[PosSelect,MaxSelect]=ginput();
PosSelect=round(PosSelect)+1;
PosFirst=mod(PosSelect,FreqPeaks);

PeaksPos=zeros(1,length(framesel));
NPeaks=3
% NPeaks=ceil((length(framesel)-PosFirst)/FreqPeaks);

PeaksPos(PosFirst+round((0:FreqPeaks:(NPeaks-1)*FreqPeaks)))=1;
...
for i=1:NPeaks
    PrePeaksIdx(PosFirst-5+(i-1)*FreqPeaks:PosFirst-3+(i-1)*FreqPeaks)=1;
end
...
for i=1:size(rois,2)   %For each ROI, I calculate the (spatial) average fluo signal
 ...
 for jj=1:NPeaks
     [MaxP,PosMP]=max(bgf(3+PosFirst+(jj-1)*FreqPeaks:50+PosFirst+(jj-1)*FreqPeaks));
     Fluo=mean(bgf((PosMP+PosFirst+(jj-1)*FreqPeaks):PosMP+PosFirst+3+(jj-1)*FreqPeaks));
     Baseline2=mean(bgf(PosFirst-7+(jj-1)*FreqPeaks:PosFirst-5+(jj-1)*FreqPeaks));
     ...
     line([-7+PosFirst+(jj-1)*FreqPeaks,-5+PosFirst+(jj-1)*FreqPeaks], [Max+Bsl Max+Bsl],'Color','k','Linewidth',3);
     line([PosMP+PosFirst+(jj-1)*FreqPeaks,PosMP+3+PosFirst+(jj-1)*FreqPeaks], [Max+Sig Max+Sig],'Color','r','Linewidth',3);
 end
